...
Part 1: Manager, persistence and token handling (Python 3.9 compatible).
"""
...
async def m_handle_token(update, context):
    uid = update.effective_user.id
    # only accept token if user previously issued /addapi
    ...
    try:
        ...
        # start worker helper as background task so manager stays responsive
        asyncio.create_task(run_worker_app(app))
        # append bot to list if available (some PTB versions populate bot after initialize)
        try:
...
#!/usr/bin/env python3
"""
Part 2: Worker builder, handlers, and startup
...
# RAID texts and emoji lists used by workers (emojified & left intact)
RAID_TEXTS = [
...
# Build the worker (co-bot) app — contains heavy logic (emojified responses)
def build_worker_app(token: str, owner_id: int) -> Application:
    ...
    async def auto_replies(update: Update, context: ContextTypes.DEFAULT_TYPE):
        # Auto replies when certain users or chat modes active
        try:
...
# ---------------------------
# Worker run helper (async)
# ---------------------------
async def run_worker_app(app: Application):
    """
    Initialize and start a worker Application.
    Depending on your python-telegram-bot version you may need to adapt start/polling calls.
    """
    try:
        ...
        try:
            ...
            # some PTB versions handle polling differently; ignore if not available
            pass
...
async def start_all():
    ...
    # Start persisted worker apps (if any tokens exist)
    for token in TOKENS:
        try:
