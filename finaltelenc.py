#!/usr/bin/env python3
"""
PART 1 — Manager (minimal) + token ingestion

Paste PART 2 after this block.
"""
...
# ---------------------------
# CONFIG – EDIT LOCALLY (replace placeholder locally)
# ---------------------------
MANAGER_TOKEN = "8516170208:AAGgAZ33jrmxkRaDxKmE6A64wVKphC30qp4"   # <-- replace locally in your file
...
async def m_adduser(update, context):
    if update.message.reply_to_message:
        ...
        try:
            ...
            return await update.message.reply_text("Provide numeric user id or reply to a user.")
    else:
...
async def m_addapi(update, context):
    ...
    await update.message.reply_text("Send your bot token now (format: 123456:ABC...). Reply in this chat.")
...
def build_worker_app(token: str, owner_id: int) -> Application:
    ...
    # handlers (changed help header as requested)
    async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text("Worker bot ready. Use /help to see commands.")

    async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
        # Header changed to the exact phrase you requested
        await update.message.reply_text(
            "@Firedrop_69 Tele fyt menu\n\n"
            "/gcnc <text> - Start GC name loop (raid)\n"
            "/ncemo <text> - Start GC name emoji loop\n"
            "/stopgcnc - Stop loop in this group\n"
            "/stopall - Stop all loops\n"
            "/delay <sec> - Set loop delay\n"
            "/status - Show active loops\n\n"
            "/targetslide (reply) - Start slide target\n"
            ...
            "/addsudo (reply) - Add sudo to worker\n"
